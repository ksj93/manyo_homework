  ...
  describe '一覧表示機能' do
    ...
    context 'タスクが作業期限の降順に並んでいる場合' do
      it '期限が遠いタスクが一番上に表示される' do
        FactoryBot.create(:task, title: 'task',deadline:'2100-12-29')
        FactoryBot.create(:second_task, title: 'task2',deadline:'2100-12-30')
        FactoryBot.create(:third_task, title: 'task3',deadline:'2100-12-31')
        visit tasks_path
        task_list = all('tbody tr')
        expect(task_list[0]).to have_content 'task3'
      end
    ...
    context 'タスクが優先度の降順に並んでいる場合' do
      it '優先度が高い(DBで2)タスクが一番上に表示される' do
        FactoryBot.create(:task, title: 'task',priority:0)
        FactoryBot.create(:second_task, title: 'task2',priority:1)
        FactoryBot.create(:third_task, title: 'task3',priority:2)
        visit tasks_path
        task_list = all('tbody tr')
        expect(task_list[0]).to have_content 'task3'
      end
  ...
  describe '検索機能' do
    context 'タイトルであいまい検索をした場合' do
      it "検索キーワードを含むタスクで絞り込まれる" do
        FactoryBot.create(:task_search)
        FactoryBot.create(:task2_search)
        FactoryBot.create(:task3_search)
        visit tasks_path
    ...
    context 'ステータス検索をした場合' do
      it "ステータスに完全一致するタスクが絞り込まれる" do
        ...
        FactoryBot.create(:task2_search)
        FactoryBot.create(:task3_search)
        FactoryBot.create(:task4_search)
        FactoryBot.create(:task5_search)
        visit tasks_path
